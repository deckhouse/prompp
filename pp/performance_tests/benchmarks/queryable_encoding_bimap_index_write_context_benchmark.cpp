...
std::shared_ptr<Lss> get_lss_after_shrink() {
  ...
  static const std::shared_ptr<ShrunkState> state = []() {
    ...
    lss.fill_touched_series_mapping(shrink_boundary, *s->snapshot_copy, s->old_to_new, lss.added_series());
    lss.finalize_copy_and_shrink(shrink_boundary, *s->snapshot_copy, s->old_to_new);
    return s;
  }();
  return state->lss;
}
