...
using entrypoint_types::LsIdsSlice;
using entrypoint_types::LsIdsSlicePtr;
...
PROMPP_ALWAYS_INLINE FindOrEmplaceResult find_or_emplace(auto& lss, const auto& label_set) {
  if constexpr (Lss::kIsReadOnly) {
    ...
    const entrypoint_types::ReallocationsDetector reallocation_detector(lss);
...
extern "C" void prompp_primitives_lss_find_or_emplace_builder(void* args, void* res) {
  ...
  new (res) FindOrEmplaceResult(std::visit(
      [&builder = in->builder]<typename Lss>(Lss& lss) {
        static const entrypoint_types::SnapshotLSS::value_type empty_label_set;
        const auto& label_set = builder.snapshot ? std::get<entrypoint_types::SnapshotLSS>(*builder.snapshot)[builder.ls_id] : empty_label_set;
...
extern "C" void prompp_primitives_group_series_by_label_names(void* args, void* res) {
  ...
  series_index::querier::group_series_by_label_names<entrypoint_types::SnapshotLSS, PromPP::Primitives::Go::Slice>(
      std::get<entrypoint_types::SnapshotLSS>(*in->snapshot), in->series_ids.span(), in->label_name_ids.span(), out->groups);
}
...
extern "C" void prompp_create_snapshot_lss(void* args, void* res) {
  ...
  new (res) Result{.snapshot = entrypoint_types::create_snapshot_lss(*static_cast<Arguments*>(args)->lss)};
}
...
extern "C" void prompp_primitives_snapshot_lss_copy_added_series(uint64_t source_snapshot,
  ...
  const auto& src_snapshot_variant = *std::bit_cast<entrypoint_types::SnapshotLSSVariant*>(source_snapshot);
  const auto& src = std::get<entrypoint_types::SnapshotLSS>(src_snapshot_variant);
  const auto& src_bitset = *std::bit_cast<BareBones::Bitset*>(source_bitset);
  auto& dst = std::get<QueryableEncodingBimap>(*std::bit_cast<entrypoint_types::LssVariant*>(destination_lss));
...
extern "C" void prompp_primitives_lss_finalize_copy_and_shrink(void* args) {
  ...
  const auto* in = static_cast<const Arguments*>(args);
  auto& lss = std::get<QueryableEncodingBimap>(*in->lss);
  auto& resolve_snapshot = std::get<entrypoint_types::SnapshotLSS>(*in->resolve_snapshot);
