...
extern "C" void prompp_remote_write_encode_message(void* args) {
  struct Arguments {
    ...
    PromPP::Primitives::Go::SliceView<entrypoint_types::SnapshotLSSVariantPtr> snapshot_list;
  ...
  const auto snapshot_getter = [in](uint32_t shard_id) -> const entrypoint_types::SnapshotLSS& {
    return std::get<entrypoint_types::SnapshotLSS>(*in->snapshot_list[shard_id]);
  };
