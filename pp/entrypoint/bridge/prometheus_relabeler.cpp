...
extern "C" void prompp_prometheus_per_goroutine_relabeler_input_relabeling(void* args, void* res) {
  ...
  try {
    std::visit(
        [in, out](auto& hashdex) {
          auto& input_lss = std::get<entrypoint_types::EncodingBimap>(*in->input_lss);
          auto& target_lss = std::get<entrypoint_types::QueryableEncodingBimap>(*in->target_lss);

          const entrypoint_types::ReallocationsDetector reallocation_detector(target_lss);
...
extern "C" void prompp_prometheus_per_goroutine_relabeler_append_relabeler_series(void* args, void* res) {
  ...
  try {
    auto& lss = std::get<entrypoint_types::QueryableEncodingBimap>(*in->target_lss);
...
extern "C" void prompp_remap_stale_nans_state(void* args) {
  struct Arguments {
    StaleNaNsStatePtr stale_nans_state;
    entrypoint_types::LsIdsSlicePtr dst_src_ls_ids_mapping;
  };
