...
func (s *HashdexSuite) TestSnappyProtobufHashdex() {
	wr := &prompb.WriteRequest{
		Timeseries: []prompb.TimeSeries{
			{
				Labels: []prompb.Label{
	...
	hlimits := cppbridge.DefaultWALHashdexLimits()
	h, err := cppbridge.NewWALSnappyProtobufHashdex(compressed, hlimits)
	s.Require().NoError(err)
	cluster := h.Cluster()
	replica := h.Replica()

	s.T().Log("check that cluster and replica stay unchanged")
	s.Equal("second_replica", replica)
