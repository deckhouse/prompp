...
func (s *PerGoroutineRelabelerSuite) TestRelabelingFromCachePartially() {
	...
	s.False(ok)
	s.Equal(uint64(4), shardsInnerSeries.DataByShard(0)[0].Size())
	...
	s.Require().NoError(err)
	s.Equal(cppbridge.RelabelerStats{1, 1, 0}, stats)
	s.Equal(uint64(5), shardsInnerSeries.DataByShard(0)[0].Size())
...
func (s *StateV2Suite) TestReconfigureWithInvalidOrderOfGenerationHead() {
	...
	// Act
	state.Reconfigure(2, 1, func(maps []*cppbridge.IdsMapping) {
		maps[0] = s.createIdsMapping()
	})
...
func (s *StateV2Suite) TestReconfigureWithShardNumberChange() {
	...
	// Act
	state.Reconfigure(1, 2, func(maps []*cppbridge.IdsMapping) {
		maps[0] = s.createIdsMapping()
	})
...
func (s *StateV2Suite) TestReconfigureWithEmptyIdsMapping() {
	...
	// Act
	state.Reconfigure(1, 1, func([]*cppbridge.IdsMapping) {})
