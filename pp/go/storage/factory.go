...
// UploadHead upload [HeadOnDisk] from [WalOnDisk].
func UploadHead(
	id, headDir string,
	releaseHeadFn func(),
	setLastAppendedSegmentID func(segmentID uint32),
	...
	for shardID := range numberOfShards {
		...
		go func(shardID uint16, shardWalFilePath string) {
			defer wg.Done()
			shardLoadResults[shardID] = UploadShard(shardWalFilePath, swn, maxSegmentSize, shardID)
...
func UploadShard(
	...
	res.corrupted = false
	res.shard = shard.NewShard(lss, dataStorage, wal.NewWal(decoder.CreateEncoder(), sw, maxSegmentSize), shardID)
