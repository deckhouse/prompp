...
// 	return s.acquireWithInserter(ctx, n, func(w waiter) *list.Element {
// 		return s.waiters.PushBack(w)
// 	})
// }

// // AcquirePriority acquires the priority semaphore with a weight of n, blocking until resources
// // are available or ctx is done. On success, returns nil. On failure, returns
// // ctx.Err() and leaves the semaphore unchanged.
// func (s *Weighted) AcquirePriority(ctx context.Context, n int64) error {
// 	return s.acquireWithInserter(ctx, n, func(w waiter) *list.Element {
// 		var elem *list.Element
// 		if s.lastPri == nil {
// 			elem = s.waiters.PushFront(w)
