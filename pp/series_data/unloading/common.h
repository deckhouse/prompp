...
template <chunk::DataChunk::Type ChunkType, class DataStorage>
...
[[nodiscard]] PROMPP_ALWAYS_INLINE DataStorage::CompactBitSequence& get_chunk_stream(DataStorage& storage, uint32_t ls_id, uint8_t chunk_id) noexcept {
  const auto& chunk_data = std::ranges::next(typename DataStorage::SeriesChunkIterator{&storage, ls_id}, chunk_id, DataStorage::SeriesChunks::end());
