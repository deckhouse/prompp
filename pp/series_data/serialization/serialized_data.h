...
struct SerializedData {
  ...
  BareBones::Memory<BareBones::MemoryControlBlock, unsigned char> bytes_buffer;
...
class DataSerializer {
 public:
  static SerializedData serialize(const DataStorage& storage, const querier::QueriedChunkList& queried_chunks) noexcept {
    return serialize_internal(storage, queried_chunks);
  }
  static SerializedData serialize(const DataStorage& storage) noexcept { return serialize_internal(storage, storage.chunks()); }
 ...
 private:
  ...
  static SerializedData serialize_internal(const DataStorage& storage, const ChunkList& chunks) noexcept {
    ...
    for (auto& chunk_data : chunks) {
      ...
      if (chunk_data.is_open()) [[likely]] {
        if (const auto& chunk = get_chunk<kOpen>(storage, chunk_data); !chunk.is_empty()) [[likely]] {
          fill_serialized_chunk<kOpen>(storage, chunk, serialized_data.chunks.emplace_back(chunk_data.series_id()), timestamp_streams_data, data_size,
                                       serialized_data.bytes_buffer);
        }
      } else {
        fill_serialized_chunk<kFinalized>(storage, get_chunk<kFinalized>(storage, chunk_data), serialized_data.chunks.emplace_back(chunk_data.series_id()),
                                          timestamp_streams_data, data_size, serialized_data.bytes_buffer);
      }
    }

    serialized_data.bytes_buffer.grow_to_fit_at_least(data_size + kReservedBytesForReader.size());
    std::memcpy(serialized_data.bytes_buffer.control_block().data + data_size, kReservedBytesForReader.data(), kReservedBytesForReader.size());
  ...
  static void fill_serialized_chunk(const DataStorage& storage,
                                    const chunk::DataChunk& chunk,
                                    chunk::SerializedChunk& serialized_chunk,
                                    TimestampStreamsData& timestamp_streams_data,
                                    uint32_t& data_size,
                                    BareBones::Memory<BareBones::MemoryControlBlock, unsigned char>& buffer) noexcept {
    ...
    switch (chunk.encoding_state.encoding_type) {
      ...
      case kDoubleConstant: {
        ...
        std::memcpy(buffer + data_size, &storage.variant_encoders[chunk.encoder.external_index].double_constant, sizeof(encoder::value::DoubleConstantEncoder));
      ...
      case kTwoDoubleConstant: {
        ...
        std::memcpy(buffer + data_size, &storage.variant_encoders[chunk.encoder.external_index].two_double_constant,
...
class SerializedDataView {
 public:
  ...
  [[nodiscard]] SerializedSeriesIterator create_current_series_iterator() const noexcept { return {data_.bytes_buffer, get_chunks(), series_index_}; }
