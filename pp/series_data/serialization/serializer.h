 ...
 private:
  ...
  void fill_serialized_chunk(const chunk::DataChunk& chunk,
    ...
    switch (chunk.encoding_state.encoding_type) {
      ...
      case kValuesGorilla: {
        fill_timestamp_stream_offset<chunk_type>(timestamp_streams_data, chunk.timestamp_encoder_state_id, serialized_chunk, data_size);
        serialized_chunk.set_offset(data_size);
        data_size += storage_.get_values_gorilla_stream<chunk_type>(chunk.encoder.external_index).size_in_bytes();
        break;
      }

      case kGorilla: {
        serialized_chunk.set_offset(data_size);
        data_size += storage_.get_gorilla_encoder_stream<chunk_type>(chunk.encoder.external_index).size_in_bytes();
        break;
      }
  ...
  void write_chunk_data(const chunk::DataChunk& chunk, TimestampStreamsData& timestamp_streams_data, std::ostream& stream) noexcept {
    ...
    switch (chunk.encoding_state.encoding_type) {
      ...
      case kValuesGorilla: {
        write_timestamp_stream<chunk_type>(timestamp_streams_data, chunk.timestamp_encoder_state_id, stream);
        write_compact_bit_sequence(storage_.get_values_gorilla_stream<chunk_type>(chunk.encoder.external_index), stream);
        break;
      }

      case kGorilla: {
        write_compact_bit_sequence(storage_.get_gorilla_encoder_stream<chunk_type>(chunk.encoder.external_index), stream);
        break;
      }
 ...
 private:
  ...
  SerializedData serialize_impl(const ChunkList& chunks) {
    ...
    for (auto& chunk_data : chunks) {
      ...
      if (chunk_data.is_open()) [[likely]] {
        if (const auto& chunk = get_chunk<kOpen>(chunk_data); !chunk.is_empty()) [[likely]] {
          fill_serialized_chunk<kOpen>(chunk, result.chunks.emplace_back(chunk_data.series_id()), timestamp_streams_data, data_size, result.bytes_buffer_);
        }
      } else {
        fill_serialized_chunk<kFinalized>(get_chunk<kFinalized>(chunk_data), result.chunks.emplace_back(chunk_data.series_id()), timestamp_streams_data,
                                          data_size, result.bytes_buffer_);
      }
    }

    result.bytes_buffer_.grow_to_fit_at_least(data_size + kReservedBytesForReader.size());
    std::memcpy(result.bytes_buffer_.control_block().data + data_size, kReservedBytesForReader.data(), kReservedBytesForReader.size());
  ...
  void fill_serialized_chunk(const chunk::DataChunk& chunk,
    ...
    switch (chunk.encoding_state.encoding_type) {
      ...
      case kDoubleConstant: {
        ...
        std::memcpy(buffer.control_block().data + data_size, &storage_.variant_encoders[chunk.encoder.external_index].double_constant,
                    sizeof(encoder::value::DoubleConstantEncoder));
        data_size += sizeof(encoder::value::DoubleConstantEncoder);
        break;
      }
      ...
      case kTwoDoubleConstant: {
        ...
        std::memcpy(buffer.control_block().data + data_size, &storage_.variant_encoders[chunk.encoder.external_index].two_double_constant,
                    sizeof(encoder::value::TwoDoubleConstantEncoder));
        data_size += sizeof(encoder::value::TwoDoubleConstantEncoder);
        break;
      }
      ...
      case kAscInteger: {
        serialized_chunk.set_offset(data_size);
        write_compact_bit_sequence(storage_.get_asc_integer_stream<chunk_type>(chunk.encoder.external_index), data_size, buffer);
        break;
      }

      case kAscIntegerThenValuesGorilla: {
        serialized_chunk.set_offset(data_size);
        write_compact_bit_sequence(storage_.get_asc_integer_then_values_gorilla_stream<chunk_type>(chunk.encoder.external_index), data_size, buffer);
        break;
      }

      case kValuesGorilla: {
        serialized_chunk.set_offset(data_size);
        write_compact_bit_sequence(storage_.get_values_gorilla_stream<chunk_type>(chunk.encoder.external_index), data_size, buffer);
        break;
      }

      case kGorilla: {
        serialized_chunk.set_offset(data_size);
        write_compact_bit_sequence(storage_.get_gorilla_encoder_stream<chunk_type>(chunk.encoder.external_index), data_size, buffer);
        break;
      }
