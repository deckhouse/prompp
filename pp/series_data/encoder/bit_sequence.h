...
struct PROMPP_ATTRIBUTE_PACKED BitSequenceWithItemsCount {
  ...
  [[nodiscard]] PROMPP_ALWAYS_INLINE static uint8_t count(const CompactBitSequence<Reallocator>& stream) noexcept {
    return bit_sequence_items_count(stream.raw_bytes());
  }
  ...
  [[nodiscard]] PROMPP_ALWAYS_INLINE static BareBones::BitSequenceReader reader(const CompactBitSequence<Reallocator>& stream) noexcept {
    return bit_sequence_reader(stream.bytes());
  }
...
template <BareBones::ReallocatorInterface Reallocator>
struct BareBones::IsTriviallyReallocatable<series_data::encoder::BitSequenceWithItemsCount<Reallocator>> : std::true_type {};
