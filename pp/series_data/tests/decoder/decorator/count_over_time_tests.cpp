...
using series_data::decoder::decorator::CountOverTimeIterator;
using series_data::encoder::Sample;

struct CountOverTimeIteratorCase {
  std::vector<Sample> samples;
  PromPP::Primitives::TimeInterval interval;
  std::vector<Sample> expected{};
};

class CountOverTimeFixture : public ::testing::TestWithParam<CountOverTimeIteratorCase> {
 protected:
...
TEST_P(CountOverTimeFixture, Test) {
  // Arrange
  ...
  Decoder::create_decode_iterator<DataChunk::Type::kOpen>(storage_, storage_.open_chunks[0], [&actual_samples]<typename Iterator>(Iterator&& begin, auto&&) {
    std::ranges::copy(CountOverTimeIterator(UniversalDecodeIterator{std::in_place_type<Iterator>, std::forward<Iterator>(begin)}, GetParam().interval),
...
TEST_P(CountOverTimeFixture, TestReset) {
  // Arrange
  std::vector<Sample> actual_samples;
  ...
  Decoder::create_decode_iterator<DataChunk::Type::kOpen>(storage_, storage_.open_chunks[0], [&actual_samples]<typename Iterator>(Iterator&& begin, auto&&) {
    ...
    CountOverTimeIterator iterator(UniversalDecodeIterator{std::in_place_type<Iterator>, std::forward<Iterator>(begin)}, GetParam().interval);
...
INSTANTIATE_TEST_SUITE_P(
    StaleNan,
    CountOverTimeFixture,
    testing::Values(CountOverTimeIteratorCase{.samples{
                                              ...
                                              .interval{.min = 0, .max = 100},
                                              .expected{Sample{.timestamp = 100, .value = 2.0}}},
