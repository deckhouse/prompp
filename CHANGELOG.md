...
### Fixes
1. **Stored XSS in React web UI (CVE-2026-40179, CVE-2026-44903).** Backported upstream fixes (GHSA-vffh-x6r8-xx99, GHSA-fw8g-cg8f-9j28) — graph tooltips, the Metrics Explorer fuzzy results, the heatmap bucket tooltip, and the heatmap y-axis tick labels now `escapeHTML` metric names and label values (including `le`) before injecting them into `innerHTML`, blocking script execution from crafted metrics ingested via scrape, remote-write, or OTLP.
