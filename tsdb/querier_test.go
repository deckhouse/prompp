...
type blockChunkSeriesSet1 struct {
	blockBaseSeriesSet1
}

func newblockChunkSeriesSet1() *blockChunkSeriesSet1 {
	bss := &blockChunkSeriesSet1{
		blockBaseSeriesSet1{},
	}

	runtime.SetFinalizer(bss, func(s *blockChunkSeriesSet1) {
		fmt.Println(" === blockChunkSeriesSet1 des")
	})

	return bss
}

type blockBaseSeriesSet1 struct {
	curr    seriesData1
	builder sbuilder
}

func (b *blockBaseSeriesSet1) Next() bool {
	dd(&b.builder)
	b.curr.labels = b.builder.Labels()
	return true
}

func (b *blockBaseSeriesSet1) reset() {
	b.builder.reset()
	b.curr.reset()
}
...
func (b *sbuilder) Labels() Labels {
	b.builder.base = newLabels()
	b.builder.del = b.builder.del[:0]

	return b.builder.base
}

func (b *sbuilder) reset() {
	b.builder.base = Labels{}
}
...
type seriesData1 struct {
	chks   []chunks.Meta
	labels Labels
}

func (b *seriesData1) reset() {
	b.labels = Labels{}
}
...
func newLabels() Labels {
	...
	// runtime.SetFinalizer(st, func(s *Snapshot) {
	// 	fmt.Println(" === Snapshot des")
	// })
...
func newLSS() *LSS {
	gcDestroyDetector := uint64(42)
	lss := &LSS{
		pointer:           42,
		gcDestroyDetector: &gcDestroyDetector,
	}

	runtime.SetFinalizer(lss, func(s *LSS) {
		fmt.Println(" === LSS des")
	})
