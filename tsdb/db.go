...
type Options struct {
	...
	// External trigger.
	ReloadBlocksExternalTrigger ReloadBlocksExternalTrigger // OP_CHANGES.md: rebuild on cpp
}
...
type DB struct {
	...
	trigger  ReloadBlocksExternalTrigger // OP_CHANGES.md: rebuild on cpp
...
func open(dir string, l log.Logger, r prometheus.Registerer, opts *Options, rngs []int64, stats *DBStats) (_ *DB, returnedErr error) {
	...
	db := &DB{
		...
		logger:         l,
		opts:           opts,
		trigger:        trigger, // OP_CHANGES.md: rebuild on cpp
