...
func FuzzParseMetricText(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, in []byte) {
		p, warning := textparse.New(in, "text/plain", false, symbolTable)
...
func FuzzParseOpenMetric(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, in []byte) {
		p, warning := textparse.New(in, "application/openmetrics-text", false, symbolTable)
		if p == nil || warning != nil {
			// An invalid content type is being passed, which should not happen
			// in this context.
			t.Skip()
...
func FuzzParseProtobuf(f *testing.F) {
	...
	for _, s := range corpus {
		f.Add(s.Data, s.ParseClassic)
	}

	f.Fuzz(func(t *testing.T, in []byte, parseClassic bool) {
		if len(in) > maxInputSize {
			t.Skip()
		}
		p := textparse.NewProtobufParser(in, parseClassic, symbolTable)
		var err error
		for {
