...
type API struct {
	...
	opHandler        *handler.OpHandler // OP_CHANGES.md: rebuild on cpp
...
func NewAPI(
	...
	hq storage.Queryable, // OP_CHANGES.md: rebuild on cpp
	receiver handler.Receiver, // OP_CHANGES.md: rebuild on cpp
	...
	a := &API{
		...
		HeadQueryable:    hq,       // OP_CHANGES.md: rebuild on cpp
		headStatusGetter: receiver, // OP_CHANGES.md: rebuild on cpp
	...
	if rwEnabled {
		a.opHandler = handler.NewOpHandler(receiver, logger, registerer) // OP_CHANGES.md: rebuild on cpp
	}
	if otlpEnabled {
...
func (api *API) Register(r *route.Router) {
	...
	r.Get("/status/tsdb", wrapAgent(api.serveHeadStatus)) // OP_CHANGES.md: rebuild on cpp
	r.Get("/status/walreplay", api.serveWALReplayStatus)
	r.Post("/read", api.ready(api.remoteRead))
	r.Post("/write", api.ready(api.opRemoteWrite(middleware.ResolveMetadataRemoteWriteVanilla))) // OP_CHANGES.md: rebuild on cpp
	r.Post("/otlp/v1/metrics", api.ready(api.otlpWrite))

	api.opRegister(r, wrapAgent) // OP_CHANGES.md: rebuild on cpp
