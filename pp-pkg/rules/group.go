...
func NewGroup(o GroupOptions) (*Group, error) {
	...
	if o.statelessRelabeler == nil {
		var err error
		o.statelessRelabeler, err = cppbridge.NewStatelessRelabeler([]*cppbridge.RelabelConfig{})
		if err != nil {
			return nil, fmt.Errorf("failed creating report stateless relabeler: %w", err)
		}
	}
	state.SetStatelessRelabeler(o.statelessRelabeler)
	state.EnableTrackStaleness()
	state.SetRelabelerOptions(&cppbridge.RelabelerOptions{HonorTimestamps: true, TrackTimestampsStaleness: true})
	...
	defer func() {
		...
		go func(now time.Time) {
			...
			case <-time.After(2 * g.interval):
				...
				g.state.SetDefTimestamp(now.Add(-g.QueryOffset()).UnixMilli())
				if err := bs.CommitWithState(ctx, g.state); err != nil {
...
func (g *Group) Eval(ctx context.Context, ts time.Time) {
	...
	if commitErr := bs.CommitWithState(ctx, g.state); commitErr != nil {
...
func (g *Group) concurrencyEval(ctx context.Context, ts time.Time, bs storage.BatchStorage) float64 {
	...
	g.state.SetDefTimestamp(ts.Add(-ruleQueryOffset).UnixMilli())
