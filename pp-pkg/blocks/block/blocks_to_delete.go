...
func deletableBlocks(
	...
	for u := range BeyondTimeRetention(retentionDuration, timeRetentions, blocks) {
		deletable[u] = struct{}{}
	}

	for u := range BeyondSizeRetention(maxBytes, extraSize, sizeRetentions, blocks) {
		deletable[u] = struct{}{}
	}
...
func CatalogHeadsSize(dir string, c *catalog.Catalog) (catalogSize int64) {
	catalogSize += c.OnDiskSize()
	heads := c.List(nil, nil)
	for _, h := range heads {
