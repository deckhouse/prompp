...
func TestXxx(t *testing.T) {
	cfgs := []*Config{
		{
			Regex:  MustNewRegexp("(j.*)"),
			Action: LabelDrop,
		},
		{
			Regex:  MustNewRegexp("(__.*)"),
			Action: LabelDrop,
		},
		{
			Regex:        MustNewRegexp("(__.*)"),
			SourceLabels: model.LabelNames{"jab"},
			TargetLabel:  "hash_mod",
			Action:       HashMod,
			Modulus:      1000,
		},
	}
...
func TestXxx2(t *testing.T) {
	// v := "Ava:lid_23name"
	v := "1alid_23name"
	ln := model.LabelName(v)

	t.Log(ln.IsValid())

	lv := model.LabelValue(v)
	t.Log(model.IsValidMetricName(lv))

	t.Log(lv.IsValid())

	vs := []string{"\xa0\xa1", "ol\xc3\xa1 mundo", "a\xc5z", "\x80", "\xFE", "\xF1"}

	for _, v := range vs {
		lv := model.LabelValue(v)
		// t.Log("IsValidMetricName:", v, model.IsValidMetricName(lv))
		t.Log("IsValid          :", v, lv.IsValid())
		if !lv.IsValid() {
			fmt.Println(v)
		}
	...
	vvv := "\x80\x8F\x90\x9Fzxca"

	t.Log(len(vvv))

	t.Log("IsValidMetricName:", v, model.IsValidMetricName("some:job2:boj"))
}

func TestXxx4(t *testing.T) {
	ls := labels.FromStrings("aaa", "111", "bbb", "")
