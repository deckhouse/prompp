...
func testRemoteWriteConfig() *config.OpRemoteWriteConfig { // OP_CHANGES.md: rebuild on cpp
	return &config.OpRemoteWriteConfig{ // OP_CHANGES.md: rebuild on cpp
...
func TestNoDuplicateWriteConfigs(t *testing.T) {
	...
	cfg1 := config.OpRemoteWriteConfig{ // OP_CHANGES.md: rebuild on cpp
		RemoteWriteConfig: config.RemoteWriteConfig{
			Name: "write-1",
			URL: &common_config.URL{
				URL: &url.URL{
	...
	cfg2 := config.OpRemoteWriteConfig{ // OP_CHANGES.md: rebuild on cpp
		RemoteWriteConfig: config.RemoteWriteConfig{
			Name: "write-2",
			URL: &common_config.URL{
				URL: &url.URL{
	...
	cfg3 := config.OpRemoteWriteConfig{
		RemoteWriteConfig: config.RemoteWriteConfig{ // OP_CHANGES.md: rebuild on cpp
			URL: &common_config.URL{
				URL: &url.URL{
					Scheme: "http",
					Host:   "localhost",
	...
	type testcase struct {
		cfgs []*config.OpRemoteWriteConfig // OP_CHANGES.md: rebuild on cpp
		err  bool
	}
...
func TestUpdateWithRegisterer(t *testing.T) {
	...
	c1 := &config.OpRemoteWriteConfig{ // OP_CHANGES.md: rebuild on cpp
		RemoteWriteConfig: config.RemoteWriteConfig{
			Name: "named",
			URL: &common_config.URL{
				URL: &url.URL{
	...
	c2 := &config.OpRemoteWriteConfig{ // OP_CHANGES.md: rebuild on cpp
		RemoteWriteConfig: config.RemoteWriteConfig{
			URL: &common_config.URL{
				URL: &url.URL{
					Scheme: "http",
	...
	conf := &config.Config{
		GlobalConfig:       config.DefaultGlobalConfig,
		RemoteWriteConfigs: []*config.OpRemoteWriteConfig{c1, c2}, // OP_CHANGES.md: rebuild on cpp
	}
	require.NoError(t, s.ApplyConfig(conf))
...
func TestWriteStorageApplyConfigsPartialUpdate(t *testing.T) {
	...
	c0 := &config.OpRemoteWriteConfig{ // OP_CHANGES.md: rebuild on cpp
		RemoteWriteConfig: config.RemoteWriteConfig{
	...
	conf := &config.Config{
		GlobalConfig:       config.GlobalConfig{},
		RemoteWriteConfigs: []*config.OpRemoteWriteConfig{c0, c1, c2}, // OP_CHANGES.md: rebuild on cpp
	}
